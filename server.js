...
app.get('/player_info/select_player', function(req, res) {
  var selected_player = pgp.ParameterizedQuery('select * from football_players where id = $1;', [req.params.id]);
  var no_games = pgp.ParameterizedQuery('select count(*) from football_players where $1 in players;', [req.params.id]);
  var query = 'select id, name from football_players;';
  // do database query
  db.task('get-everything', task => {
        return task.batch([
            task.many(selected_player),
            task.one(no_games),
            task.many(query)
        ]);
    })
  // once database query is done, render page as below with meaningful data
    .then(info => {
    	res.render('pages/player_info',{
...
app.get('/team_stats', function(req, res) {
  var retreive = 'select * from football_games;';
